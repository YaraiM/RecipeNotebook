...
- 作成したレシピの一覧がカード形式で表示されます。
- 上部の検索フォームで、▽レシピ／材料名でのキーワード検索、▽お気に入りレシピの検索、作成・更新日での範囲検索を行えます。
- 新規作成ボタンをクリックすると、新規作成フォームに遷移します。
- カードをクリックするとレシピ詳細画面に遷移します。
- カードの左上の★マークをクリックするとお気に入り登録・解除ができます。
- カード右上の🖊マークをクリックすると直接編集画面に遷移できます。
- カードの右上の×マークをクリックすると削除モーダルが表示され、レシピを削除できます。

### 新規作成・編集画面

- フォームに入力した情報に基づき、レシピの新規作成・編集を行えます。
- 編集フォームには、登録済みのレシピ情報があらかじめ入力されています。
- 許可されていない入力情報でフォーム送信を行うとバリデーションエラーが発生します。
...
- バックエンドはREST
  APIを提供し、フロントエンドはAPIを呼び出して画面に表示する役割に終始するようにしています。基本機能だけでなく、例外処理で表示されるメッセージも、基本的にはREST
  APIから提供されるエラーメッセージを参照しています。
- 役割を明確に分離することで、バックエンド、フロントエンドともに将来の機能拡張を容易にすることが狙いです。
...
- データベースへの情報登録だけでなく、画像ファイルをアップロードできるようにしました。
- 環境変数に設定したホームディレクトリに画像アップロード用のディレクトリを作成し、そこに画像を保存します。保存した画像へのパスをレシピに登録することで、レシピと画像を紐づけています。
- フロントエンド側で画像ファイルをBase64にエンコードし、JSON形式で渡すことで、バックエンド側でファイルサイズやデータ形式のチェックを行えるようにしました。
- Base64のエンコード／デコードによるファイルサイズの増加とパフォーマンスの低下が課題です。今後はmultipart/form-dataによるデータ転送を検討します。

## 機能開発ごとの自動テスト実装およびCIの導入

- 実際のプロジェクトにおける共同開発を意識して、機能を実装したら都度JUnitによる単体テストを作るよう心掛けました。
- バックエンド開発を一通り終えてから、結合テストを行いました。
- また、GitHub Actionsを使用してpush時にもテストが実行されるようにしています。
- これにより、バグや想定しない動作を早期発見できるようにしました。
- 厳密な検証を行うテストコードが多めなので、コードの修正が発生したときにテスト失敗になるケースが多く、大変でした。テスト容易性は今後の課題と考えています。

## SpringSecurityの導入

- SpringSecurityを使ってログイン認証とCSRF対策を有効化しています。
- バックエンド側でCSRFトークンを取得するエンドポイントを作成し、POST/PUT/DELETEリクエスト時にフロントエンド側でトークンを呼び出し、ヘッダー情報として付加しています。
- ユーザーの管理機能を実装しておらず、管理者やユーザーごとに操作権限（認可）を分けるところまでは実装できていません。これは今後の課題です。
- HTTPSで通信の暗号化までできていないため、厳密なセキュリティ対策まではできていません。
...
- スクールで学ばなかった未知の機能については自分で調べながら実装しました。これは、開発を自走する練習になったと感じています。
- 細かな反省点を挙げるとキリがありませんが、一番の反省点は、要件定義やスケジュールの見通しが甘かったことです。
- いざ開発を始めると、当初の要件定義で想定していなかった事項がたくさん出てきて、何度も手戻りが発生しました。
- その結果、当初は企画から開発完了まで1か月半くらいを見込んでいたところ、実際には2か月半（合計180~
  190時間程度）かかりました。最初の要件定義がいかに大事か、身に染みてよくわかりました・・・
- まだまだ至らない部分がたくさんあると思いますが、ゼロからアプリケーションを作り、形にできたことは自信につながりました。引き続き、技術のキャッチアップを怠ることなく学習を続けたいと思います。