...
- フロントエンドは体系的に学習しなかったため、画面上でアプリが動作するまでにかなり時間を費やした割に、可読性がよくありません。
- 私はバックエンドエンジニアを目指していますが、ユーザーフレンドリーなアプリケーション開発ができるよう、フロントエンドの学習も進めたいと思います。
...
- ▽既存情報を更新、▽材料・調理手順の一部追加・削除、▽既存の画像ファイルを削除して新規ファイルを登録――など、様々なケースを考えなければならず、実装が大変でした。
- 一つ一つの処理を実装するたびに動作テストをして、わからなければ調べて・・・を繰り返して、想定どおりの動作を実現することができました。
...
- その結果、当初は企画から開発完了まで1か月半くらいを見込んでいたところ、実際には2か月半（合計200時間程度）かかりました。最初の要件定義がいかに大事か、身に染みてよくわかりました・・・
